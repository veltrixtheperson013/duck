...
function getPublicGuildSettings(settings = {}, configuredModel = "", now = Date.now()) {
  ...
  return {
    ...
    ticketOptions: Array.isArray(settings.ticketOptions) ? settings.ticketOptions.filter((item) => item && /^[a-z0-9_-]{1,24}$/.test(item.id || "") && typeof item.label === "string").slice(0, plus ? 10 : 5).map((item) => ({ id: item.id, label: item.label.slice(0, 80), description: typeof item.description === "string" ? item.description.slice(0, 200) : "", emoji: typeof item.emoji === "string" ? item.emoji.slice(0, 32) : "", type: item.type === "verification" ? "verification" : "support", verificationLabel: typeof item.verificationLabel === "string" ? item.verificationLabel.slice(0, 80) : "", additionalMessage: typeof item.additionalMessage === "string" ? item.additionalMessage.slice(0, 500) : "" })) : [],
...
function makeSettingsPatch(current, input, configuredModel = "", now = Date.now()) {
  ...
  if ("ticketOptions" in input) {
    ...
    patch.ticketOptions = input.ticketOptions.map((item, index) => { if (!item || typeof item !== "object" || Array.isArray(item) || Object.keys(item).some((key) => !["id", "label", "description", "emoji", "type", "verificationLabel", "additionalMessage"].includes(key)) || !/^[a-z0-9_-]{1,24}$/.test(item.id || "") || typeof item.label !== "string" || typeof item.description !== "string" || typeof item.emoji !== "string" || (item.type !== undefined && !["support", "verification"].includes(item.type)) || (item.verificationLabel !== undefined && typeof item.verificationLabel !== "string") || (item.additionalMessage !== undefined && typeof item.additionalMessage !== "string")) throw new TypeError(`Ticket option ${index + 1} is invalid.`); const label = item.label.trim(); const description = item.description.trim(); const emoji = item.emoji.trim(); const type = item.type === "verification" ? "verification" : "support"; const verificationLabel = String(item.verificationLabel || "").trim(); const additionalMessage = String(item.additionalMessage || "").trim(); if (!label || label.length > 80 || description.length > 200 || emoji.length > 32 || verificationLabel.length > 80 || additionalMessage.length > 500) throw new TypeError(`Ticket option ${index + 1} has invalid text.`); return { id: item.id, label, description, emoji, type, verificationLabel, additionalMessage }; });
  }
