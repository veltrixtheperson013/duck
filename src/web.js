  ...
  const server = http.createServer({ maxHeaderSize: 16 * 1024 }, async (req, res) => {
    ...
    try {
      ...
      if (pathname === "/api/guilds" && method === "GET") {
        ...
        return json(res, 200, { guilds: guilds.map((guild) => ({ id: guild.id, name: guild.name, icon: guild.icon || null, owner: Boolean(guild.owner), canManage: hasManageGuildPermission(guild), isAdministrator: hasAdministratorPermission(guild), botPresent: Boolean(client?.guilds?.cache?.has(guild.id)), inviteUrl: makeBotInviteUrl(guild.id, clientId()) })), cached: Boolean(cachedGuilds), stale, retryAfterMs });
      }
