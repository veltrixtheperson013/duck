import { LANGUAGES, multilingualEnabled, MULTILINGUAL_RELEASE_LABEL } from "./languages.js";
...
const pages = new Map([
  ["/icons.svg", { file: "icons.svg", type: "image/svg+xml" }],
  ["/languages.svg", { file: "languages.svg", type: "image/svg+xml" }],
  ["/lucide-LICENSE.txt", { file: "lucide-LICENSE.txt", type: "text/plain; charset=utf-8" }],
...
function securityHeaders(contentType) { return { "Content-Type": contentType, "Content-Security-Policy": "default-src 'none'; style-src 'self'; script-src 'self'; connect-src 'self'; img-src 'self' data: https://cdn.discordapp.com; frame-src https://discord.com/widget; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'; require-trusted-types-for 'script'", "Cross-Origin-Opener-Policy": "same-origin", "Cross-Origin-Resource-Policy": "same-origin", "Origin-Agent-Cluster": "?1", "Permissions-Policy": "camera=(), microphone=(), geolocation=(), payment=(), usb=()", "Referrer-Policy": "no-referrer", "Strict-Transport-Security": "max-age=31536000; includeSubDomains", "X-Content-Type-Options": "nosniff", "X-Frame-Options": "DENY", "X-Permitted-Cross-Domain-Policies": "none" }; }
...
function createDuckWebsiteServer(options = {}) {
  ...
  const server = http.createServer({ maxHeaderSize: 16 * 1024 }, async (req, res) => {
    ...
    try {
      ...
      if (pathname === "/api/languages" && method === "GET") return json(res, 200, { languages: LANGUAGES, enabled: multilingualEnabled(), release: MULTILINGUAL_RELEASE_LABEL });
      ...
      if (pathname === "/auth/logout" && method === "POST") { const auth = getSession(req); if (!auth) return json(res, 401, { error: "No active session." }); if (!requireCsrf(req, auth)) return json(res, 403, { error: "Invalid request token." }); sessions.delete(auth.id); for (const [id, operatorSession] of operatorSessions) if (operatorSession.discordSessionId === auth.id) operatorSessions.delete(id); return json(res, 200, { ok: true }, method, { "Set-Cookie": [cookie("duck_session", "", 0), operatorCookie("", 0)] }); }
      ...
      if (settingsMatch && method === "PUT") {
        ...
        for (const item of result.patch.customActions || []) if (item.roleId && !roleIds.has(item.roleId)) return json(res, 400, { error: "Every custom-action role must be safely assignable by Duck." });
