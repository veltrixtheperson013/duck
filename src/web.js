...
function sendAsset(req, res, page, method) { const asset = assetCache.get(page.file); if (!asset) return send(res, 500, "text/plain; charset=utf-8", "Duck could not load this page.", method); const isHtml = page.type.startsWith("text/html"); const cacheControl = isHtml ? "no-cache" : page.type === "image/svg+xml" ? "public, max-age=604800, stale-while-revalidate=86400" : "public, max-age=3600, stale-while-revalidate=86400"; const zipped = /(?:^|,)\s*gzip\s*(?:,|$)/i.test(req.headers["accept-encoding"] || ""); const etag = zipped ? asset.gzipEtag : asset.etag; const common = { ...securityHeaders(page.type), "Cache-Control": cacheControl, ETag: etag, Vary: "Accept-Encoding", ...(zipped ? { "Content-Encoding": "gzip" } : {}) }; if (req.headers["if-none-match"] === etag) { res.writeHead(304, common); return res.end(); } const body = zipped ? asset.gzip : asset.body; res.writeHead(200, { ...common, "Content-Length": body.length }); res.end(method === "HEAD" ? undefined : body); }
...
function createDuckWebsiteServer(options = {}) {
  ...
  async function guildAccess(req, guildId, fresh = false) { const auth = getSession(req); if (!auth) return { error: "Sign in with Discord first.", status: 401 }; const hasValidGuildCache = Array.isArray(auth.session.guilds) && auth.session.guildsAt > Date.now() - 60_000 && auth.session.guildsBackoffUntil <= Date.now(); const isBackedOff = auth.session.guildsBackoffUntil > Date.now(); let guilds; if (fresh && isBackedOff) return { error: "Discord is rate limiting permission refreshes. Try again shortly.", status: 503, retryAfterMs: Math.max(0, auth.session.guildsBackoffUntil - Date.now()) }; if (!fresh && hasValidGuildCache) guilds = auth.session.guilds; else if (isBackedOff) { if (!Array.isArray(auth.session.guilds)) return { error: "Discord is rate limiting guild refreshes. Try again shortly.", status: 503, retryAfterMs: Math.max(0, auth.session.guildsBackoffUntil - Date.now()) }; guilds = auth.session.guilds; } else { guilds = await discordApi(auth.session, "/users/@me/guilds"); } auth.session.guilds = guilds; auth.session.guildsAt = Date.now(); const guild = guilds.find(({ id }) => id === guildId); if (!guild) return { error: "You are not a member of that server.", status: 403 }; if (!hasManageGuildPermission(guild)) return { error: "Manage Server permission is required.", status: 403 }; const botGuild = client?.guilds?.cache?.get(guildId); if (!botGuild) return { error: "Invite Duck to this server before configuring it.", status: 409 }; const isAdministrator = hasAdministratorPermission(guild); let member = null; if (!isAdministrator) { member = botGuild.members?.cache?.get(auth.session.user.id) ?? (botGuild.members?.fetch ? await botGuild.members.fetch({ user: auth.session.user.id, force: true }).catch(() => null) : null); if (!member) return { error: "Duck could not verify your current server membership.", status: 403 }; } return { ...auth, guild, botGuild, member, isAdministrator }; }
  ...
  const server = http.createServer({ maxHeaderSize: 16 * 1024 }, async (req, res) => {
    ...
    try {
      ...
      if (pathname === "/api/guilds" && method === "GET") { const auth = getSession(req); if (!auth) return json(res, 401, { error: "Sign in with Discord first." }); const url = new URL(req.url, "http://duck.local"); const forceRefresh = url.searchParams.get("refresh") === "1"; const isBackedOff = Boolean(auth.session.guildsBackoffUntil) && auth.session.guildsBackoffUntil > Date.now(); const hasFreshCache = Array.isArray(auth.session.guilds) && auth.session.guildsAt > Date.now() - 60_000 && !isBackedOff; let guilds; if (isBackedOff) return json(res, 503, { error: "Discord is rate limiting guild refreshes. Try again shortly.", retryAfterMs: Math.max(0, auth.session.guildsBackoffUntil - Date.now()) }, method, { "Retry-After": String(Math.max(1, Math.ceil((auth.session.guildsBackoffUntil - Date.now()) / 1000))) }); if (!forceRefresh && hasFreshCache) guilds = auth.session.guilds; else { guilds = await discordApi(auth.session, "/users/@me/guilds"); auth.session.guilds = guilds; auth.session.guildsAt = Date.now(); } return json(res, 200, { guilds: guilds.map((guild) => ({ id: guild.id, name: guild.name, icon: guild.icon || null, owner: Boolean(guild.owner), canManage: hasManageGuildPermission(guild), isAdministrator: hasAdministratorPermission(guild), botPresent: Boolean(client?.guilds?.cache?.has(guild.id)), inviteUrl: makeBotInviteUrl(guild.id, clientId()) })) }); }
      ...
      if (settingsMatch && method === "PUT") { const access = await guildAccess(req, settingsMatch[1], true); if (access.error) return json(res, access.status, { error: access.error }); if (!requireCsrf(req, access)) return json(res, 403, { error: "Invalid request token." }); const input = await readJsonBody(req, 256 * 1024); const stored = getGuildSettings(settingsMatch[1]); const current = withOwnerPlus(stored, access.session.user.id); let result; try { result = makeSettingsPatch(current, input, process.env.OPENROUTER_MODEL); } catch (error) { if (error instanceof TypeError) return json(res, error.status || 400, { error: error.message }); throw error; } const channelIds = new Set(getGuildTextChannels(client, settingsMatch[1], access.member, access.isAdministrator).map(({ id }) => id)); for (const key of ["modChannelId", "welcomeChannelId", "automodHoneypotChannelId", "aiScanFlagChannelId"]) if (result.patch[key] != null && !channelIds.has(result.patch[key])) return json(res, 400, { error: `${key} must be visible to both you and Duck in this server.` }); for (const id of result.patch.aiScanChannelIds || []) if (!channelIds.has(id)) return json(res, 400, { error: "Every AI scan channel must be visible to both you and Duck in this server." }); if (result.patch.entryChannels?.logChannelId != null && !channelIds.has(result.patch.entryChannels.logChannelId)) return json(res, 400, { error: "logChannelId must be visible to both you and Duck in this server." }); for (const item of result.patch.automodChannelSlowmodes || []) if (!channelIds.has(item.channelId)) return json(res, 400, { error: "Every channel rate guard must be visible to both you and Duck in this server." }); for (const item of result.patch.customActions || []) if (item.channelId && !channelIds.has(item.channelId)) return json(res, 400, { error: "Every custom-action channel must be visible to both you and Duck in this server." }); const changesAdminPolicy = ("capabilityMode" in result.patch && result.patch.capabilityMode !== (current.capabilityMode || "ask")) || ("commandPrefix" in result.patch && result.patch.commandPrefix !== (current.commandPrefix || "!")); const changesAutomation = Object.keys(input).some((key) => key.startsWith("automod") || key.startsWith("aiScan") || key === "customActions"); if ((changesAdminPolicy || changesAutomation) && !access.isAdministrator) return json(res, 403, { error: "Administrator permission is required to change approval policy, AutoMod, AI scanning, or custom actions." }); if (result.patch.capabilityMode === "agent" && result.patch.capabilityMode !== current.capabilityMode) return json(res, 400, { error: "Agent mode must be enabled through Discord's Administrator confirmation flow." }); const ownerSubscription = current.subscription?.provider === "owner" ? { subscription: { ...current.subscription, updatedAt: new Date().toISOString() } } : {}; updateGuildSettings(settingsMatch[1], { ...ownerSubscription, ...result.patch }); return json(res, 200, { ok: true, settings: result.settings }); }
