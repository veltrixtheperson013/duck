...
function createDuckWebsiteServer(options = {}) {
  ...
  const server = http.createServer({ maxHeaderSize: 16 * 1024 }, async (req, res) => {
    ...
    try {
      ...
      if (pathname === "/api/site-config" && method === "GET") { const plusEnabled = isPlusEnabled(); return json(res, 200, { plusEnabled, billingConfigured: Boolean(plusEnabled && stripe && isStripeServerConfigured()) }, method); }
