...
const funSettingNames = ["funCommandsEnabled", "funQuackEnabled", "funDuckFactEnabled", "funCoinflipEnabled", "funShipEnabled", "funCurseEnabled", "funSpinwheelEnabled", "funRollEnabled", "funEightballEnabled", "funQuoteEnabled", "funRoastEnabled", "funComplimentEnabled", "funChooseEnabled", "funRateEnabled", "funWouldYouRatherEnabled"];
...
function closeCustomSelects(except = null) { for (const control of $$(".custom-select.is-open")) { if (control === except) continue; control.classList.remove("is-open"); control.querySelector("button").setAttribute("aria-expanded", "false"); } }
...
function enhanceSelect(select) {
  ...
  const control = document.createElement("div"); const trigger = document.createElement("button"); const label = document.createElement("span"); const arrow = document.createElement("i"); const menu = document.createElement("div"); control.className = "custom-select"; trigger.type = "button"; trigger.className = "custom-select-trigger"; trigger.setAttribute("aria-haspopup", "listbox"); trigger.setAttribute("aria-expanded", "false"); arrow.setAttribute("aria-hidden", "true"); trigger.append(label, arrow); menu.className = "custom-select-menu"; menu.setAttribute("role", "listbox"); control.append(trigger, menu); select.classList.add("native-select"); select.after(control); trigger.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); const opening = !control.classList.contains("is-open"); closeCustomSelects(control); control.classList.toggle("is-open", opening); trigger.setAttribute("aria-expanded", String(opening)); }); refreshCustomSelect(select);
}
...
function syncActionFields(card) { const trigger = card.querySelector("[data-action-trigger]").value; const type = card.querySelector("[data-action-type]").value; card.querySelector("[data-trigger-value-row]").hidden = trigger === "message"; card.querySelector("[data-action-response-row]").hidden = !["reply", "react"].includes(type); }
...
function configureLoyalty(loyalty = { level: "free", progress: 0, customActionLimit: 5 }, subscription = {}) {
  ...
  const current = plus ? ["Premium AI and ElevenLabs", `${loyalty.customActionLimit === null ? "Unlimited" : loyalty.customActionLimit} custom actions`, `${loyalty.memoryReplies} remembered Duck replies`] : ["Core moderation and free AI", "5 custom actions", "6 remembered Duck replies"];
...
async function openSettings(guild) {
  ...
  try {
    ...
    state.catalog = normalizeCatalog(data.models, settings); state.channels = data.channels ?? []; configurePlan(data, settings, subscription, plus, guild);
    $("[data-dialog-title]").textContent = guild.name;
    for (const key of ["aiChatEnabled", "aiVisionEnabled", "ttsEnabled", "ttsAnnounceNames", "automodEnabled", "automodHoneypotEnabled", "automodSwearFilter", "automodNsfwFilter", ...funSettingNames]) form[key].checked = settings[key];
    for (const key of ["capabilityMode", "commandPrefix", "aiChannelMode", "aiContextMode", "aiResponseStyle", "aiPersonality", "welcomeMessage", "farewellMessage", "automodEscalation", "automodGlobalSlowmodeSeconds", "automodViolationsBeforeWarn", "automodWarningsBeforeAction"]) form[key].value = settings[key]; form.automodCustomWords.value = (settings.automodCustomWords || []).join(", ");
    ...
    for (const select of $$('[data-channel-select]')) fillChannelSelect(select, state.channels, settings[select.name]); renderSlowmodes(settings.automodChannelSlowmodes); renderCustomActions(settings.customActions);
    for (const input of $$('[data-settings-panel="automod"] input, [data-settings-panel="automod"] textarea, [data-settings-panel="automod"] select, [data-settings-panel="automod"] button, [data-settings-panel="actions"] input, [data-settings-panel="actions"] select, [data-settings-panel="actions"] button')) input.disabled = !data.isAdministrator; form.automodCustomWords.disabled = !data.isAdministrator || !plus;
...
$("[data-add-slowmode]").addEventListener("click", () => addSlowmodeRow()); $("[data-add-custom-action]").addEventListener("click", () => addCustomActionCard());
const accountTrigger = $("[data-account-trigger]"); const accountMenu = $("[data-account-menu]"); accountTrigger.addEventListener("click", (event) => { event.stopPropagation(); const open = accountMenu.hidden; accountMenu.hidden = !open; accountTrigger.setAttribute("aria-expanded", String(open)); }); accountMenu.addEventListener("click", (event) => event.stopPropagation()); document.addEventListener("click", () => { accountMenu.hidden = true; accountTrigger.setAttribute("aria-expanded", "false"); });
...
$("[data-settings-form]").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const automationSettings = state.isAdministrator ? { automodEnabled: form.automodEnabled.checked, automodHoneypotEnabled: form.automodHoneypotEnabled.checked, automodHoneypotChannelId: channelValue("automodHoneypotChannelId"), automodSwearFilter: form.automodSwearFilter.checked, automodNsfwFilter: form.automodNsfwFilter.checked, ...(state.activePlus ? { automodCustomWords: form.automodCustomWords.value.split(",").map((value) => value.trim()).filter(Boolean) } : {}), automodGlobalSlowmodeSeconds: Number(form.automodGlobalSlowmodeSeconds.value), automodChannelSlowmodes: serializeSlowmodes(), automodViolationsBeforeWarn: Number(form.automodViolationsBeforeWarn.value), automodWarningsBeforeAction: Number(form.automodWarningsBeforeAction.value), automodEscalation: form.automodEscalation.value, customActions: serializeCustomActions() } : {};
