...
function enhanceSelect(select) {
  ...
  const control = document.createElement("div"); const trigger = document.createElement("button"); const label = document.createElement("span"); const arrow = document.createElement("i"); const menu = document.createElement("div"); control.className = "custom-select"; trigger.type = "button"; trigger.className = "custom-select-trigger"; trigger.setAttribute("aria-haspopup", "listbox"); trigger.setAttribute("aria-expanded", "false"); arrow.textContent = "⌄"; trigger.append(label, arrow); menu.className = "custom-select-menu"; menu.setAttribute("role", "listbox"); control.append(trigger, menu); select.classList.add("native-select"); select.after(control); trigger.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); const opening = !control.classList.contains("is-open"); closeCustomSelects(control); control.classList.toggle("is-open", opening); trigger.setAttribute("aria-expanded", String(opening)); }); refreshCustomSelect(select);
}
...
async function openSettings(guild) {
  ...
  try {
    ...
    for (const key of ["aiChatEnabled", "aiVisionEnabled", "ttsEnabled", "ttsAnnounceNames", ...funSettingNames]) form[key].checked = settings[key];
