...
async function api(url, options = {}) { const headers = { Accept: "application/json", ...options.headers }; if (state.me?.csrf && !["GET", "HEAD"].includes(options.method || "GET")) headers["X-Duck-CSRF"] = state.me.csrf; const response = await fetch(url.startsWith("/") ? url : `/${url}`, { ...options, headers }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || `Request failed (${response.status}).`); return body; }
...
function guildIcon(guild) {
  ...
  const image = document.createElement("img"); image.src = `https://cdn.discordapp.com/icons/${encodeURIComponent(guild.id)}/${encodeURIComponent(guild.icon)}.webp?size=128`; image.alt = ""; return image;
}
...
async function openSettings(guild) {
  ...
  try {
    ...
    fillModelSelect(form.aiModel, data.models.ai, settings.aiModel, plus); fillModelSelect(form.ttsModel, data.models.tts, settings.ttsModel, plus);
...
async function initialize() {
  try {
    const data = await api("api/me"); state.me = data; const profile = $("[data-user-area]"); profile.replaceChildren(); const avatar = data.user.avatar ? document.createElement("img") : document.createElement("span");
    ...
    state.guilds = (await api("api/guilds")).guilds; $("[data-stat-total]").textContent = state.guilds.length; $("[data-stat-manage]").textContent = state.guilds.filter((guild) => guild.canManage).length; $("[data-stat-installed]").textContent = state.guilds.filter((guild) => guild.botPresent).length; $("[data-dashboard]").hidden = false; renderGuilds();
  } catch (error) { if (/sign in|session/i.test(error.message)) $("[data-login]").hidden = false; else notice(error.message, true); }
}
...
$("[data-cancel]").addEventListener("click", () => $("[data-settings-dialog]").close()); $("[data-settings-form]").aiModel.addEventListener("change", updateDisclaimers); $("[data-settings-form]").ttsModel.addEventListener("change", updateDisclaimers);
